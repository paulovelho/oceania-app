...
export class BreadcrumbComponent {
	...
	constructor(private _router: Router, 
		...
		this.prev = this.config.pageTitle;
		this._router.events.subscribe(event => {
			if (event instanceof NavigationEnd) {
				...
				let pageTitle = this.config.pageTitle + " " + this.title;
	...
	parseRoute(node: ActivatedRouteSnapshot) { 
		let breadcrumb = node.data['breadcrumb'];
		if ( Array.isArray(breadcrumb) ) {
			breadcrumb.map(b => { this.parseSingleBread(b) });
		} else {
			this.parseSingleBread(breadcrumb);
		}
		if (node.firstChild) {
